...
async function showView(index){
  ...
  state.view=next;panel.dataset.view=def.id;state.cursor=Math.max(0,state.frames.length-1);state.transition=null;if(typeof eWarmCurrentView==='function')eWarmCurrentView();render();
}
...
async function init(){
  ...
  panel.dataset.ready='true';boot.classList.add('off');window.__RDR__=state;render();enrichmentPromise.then(()=>render());
  if(!verifyMode)await loadSevere().catch(e=>state.errors.push(String(e)));state.cursor=Math.max(0,state.frames.length-1);deriveHome();if(typeof eWarmCurrentView==='function')eWarmCurrentView();render();startAnimation();scheduleRotation();startPolling();
