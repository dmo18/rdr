...
function cloudCodeValue(code){
  const s=String(code||'').toUpperCase();if(['CLR','SKC'].includes(s))return 0;if(s==='FEW')return .18;if(s==='SCT')return .42;if(s==='BKN')return .72;if(['OVC','VV'].includes(s))return 1;const n=parseFloat(s);return Number.isFinite(n)?clamp(n/8,0,1):null;
}
async function loadWeather(){
  try{
    const headers={Accept:'application/geo+json'};
    const p=await fetchJson(`${CFG.nws}/points/${CFG.home.lat.toFixed(4)},${CFG.home.lon.toFixed(4)}`,headers),pp=p?.properties||{},stationsUrl=pp.observationStations;
    ...
    const obs=await fetchJson(`${station}/observations/latest`,headers),q=obs.properties||{},temp=cToF(q.temperature?.value),dew=cToF(q.dewpoint?.value),rh=Number.isFinite(q.relativeHumidity?.value)?Math.round(q.relativeHumidity.value):null;
    ...
    const pressure=Number.isFinite(q.barometricPressure?.value)?q.barometricPressure.value/3386.389:null,visibility=Number.isFinite(q.visibility?.value)?q.visibility.value/1609.344:null;
    let forecast=null;
    if(pp.forecastHourly){
      try{
        const fj=await fetchJson(pp.forecastHourly,headers),period=fj?.properties?.periods?.[0];
        if(period)forecast={temp:period.temperature,pop:period.probabilityOfPrecipitation?.value,windSpeed:period.windSpeed,windDirection:period.windDirection,shortForecast:period.shortForecast||'',startTime:period.startTime?new Date(period.startTime):null};
      }catch(e){state.errors.push(`hourly: ${e}`)}
    }
