...
async function loadWeather(){
  try{
    const headers={Accept:'application/geo+json'},p=await fetchJson(`${CFG.nws}/points/${CFG.home.lat.toFixed(4)},${CFG.home.lon.toFixed(4)}`,headers),pp=p?.properties||{},stationsUrl=pp.observationStations;if(!stationsUrl)throw new Error('no NWS stations endpoint');const stations=await fetchJson(stationsUrl,headers),station=stations?.features?.[0]?.id;if(!station)throw new Error('no NWS station');const obs=await fetchJson(`${station}/observations/latest`,headers),q=obs.properties||{},temp=cToF(q.temperature?.value),dew=cToF(q.dewpoint?.value),rh=Number.isFinite(q.relativeHumidity?.value)?Math.round(q.relativeHumidity.value):null,windMph=Number.isFinite(q.windSpeed?.value)?Math.round(q.windSpeed.value*2.23694):null,windDir=Number.isFinite(q.windDirection?.value)?dir8(q.windDirection.value):null,layers=Array.isArray(q.cloudLayers)?q.cloudLayers:[],cloudCover=layers.reduce((m,l)=>Math.max(m,cloudCodeValue(l?.amount)||0),0),cloudCode=layers.map(l=>l?.amount).filter(Boolean).at(-1)||((q.textDescription||'').toLowerCase().includes('clear')?'CLR':''),pressure=Number.isFinite(q.barometricPressure?.value)?q.barometricPressure.value/3386.389:null,visibility=Number.isFinite(q.visibility?.value)?q.visibility.value/1609.344:null;let forecast=null;
