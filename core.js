...
const CFG={
  ...
  mrmsBucket:'https://noaa-mrms-pds.s3.amazonaws.com',radarProduct:'MergedReflectivityQCComposite_00.50',severeProducts:{lightning:'LightningProbabilityNext30minGrid_scale_1',mesh:'MESH_00.50'},
  reference:'https://mapservices.weather.noaa.gov/static/rest/services/nws_reference_maps/nws_reference_map/MapServer',warnings:'https://mapservices.weather.noaa.gov/eventdriven/rest/services/WWA/watch_warn_adv/MapServer',tropics:'https://mapservices.weather.noaa.gov/tropical/rest/services/tropical/NHC_tropical_weather_summary/MapServer',surfaceObs:'https://mapservices.weather.noaa.gov/vector/rest/services/obs/surface_obs/MapServer',nws:'https://api.weather.gov'
};
...
function fitPanel(){const s=Math.min(innerWidth/CFG.width,innerHeight/CFG.height);panel.style.transform=`translate(-50%,-50%) scale(${Math.max(.2,s)})`}
function view(){return CFG.views[state.view]}
function lon360(lon){return lon<0?lon+360:lon}
function mapXY(lat,lon,def=view()){const[w,s,e,n]=def.bbox;return{x:(lon-w)/(e-w)*CFG.width,y:CFG.top+(n-lat)/(n-s)*CFG.mapHeight}}
