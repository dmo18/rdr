...
const legacyApis=()=>{
  for(const [owner,key] of [[Promise,'allSettled'],[Array.prototype,'at'],[Array.prototype,'flat'],[window,'DecompressionStream'],[window,'AbortController']])try{Object.defineProperty(owner,key,{value:undefined,configurable:true})}catch(e){}
...
try{
  ...
  const result=await page.evaluate(()=>({ready:panel.dataset.ready,bootOff:boot.className.includes('off'),radar:panel.dataset.radar,settled:typeof Promise.allSettled,at:typeof [].at,flat:typeof [].flat}));await page.close();
  if(errors.length||result.ready!=='true'||!result.bootOff||result.settled!=='function'||result.at!=='function'||result.flat!=='function')throw new Error(`legacy bootstrap failed: ${JSON.stringify({result,errors})}`);
