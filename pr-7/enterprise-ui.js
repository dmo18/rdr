function eHeader() {
  ...
  if (local) {
    ...
    ctx.fillText(`${warningCode(local)} WARNING AT HOME`, 252, 19.4);
  }
  ctx.textAlign = 'right';
...
function eHomeText() {
  ...
  return {
    main: 'DRY',
    sub: 'NO RAIN NEAR HOME'
  };
}
function eHazardText() {
  const warnings = eWarningsInView().filter(x => ['TOR', 'SVR', 'FFW'].includes(x.code)),
  ...
  let main = 'NONE',
    ...
    accent = '#8da4ad';
  if (warnings.length) {
    const priority = ['TOR', 'SVR', 'FFW'].find(c => warnings.some(w => w.code === c));
    main = `${priority} ${warnings.filter(w => w.code === priority).length}`;
    sub = 'NWS WARNING IN VIEW';
    accent = eHazardColor(priority);
  } else if (ltg != null && ltg >= 60) {
...
function eFooter() {
  ctx.fillStyle = 'rgba(3,9,13,.96)';
  ctx.fillRect(0, E_FOOT_TOP, CFG.width, CFG.height - E_FOOT_TOP);
  ctx.strokeStyle = 'rgba(123,154,166,.30)';
  ctx.lineWidth = .6;
  ctx.beginPath();
  ...
  ctx.fillText('HOME IMPACT', 8, 226);
  ctx.fillStyle = a.main === 'DRY' ? '#7fdcf7' : a.main === 'LIGHT' ? '#78e59a' : a.main === 'RAIN' ? '#ffe05b' : a.main === 'HEAVY' ? '#ff8b52' : '#ff5e74';
  ctx.font = '900 8.4px Arial,Helvetica,sans-serif';
  ctx.fillText(a.main, 8, 237);
  ctx.fillStyle = '#c0cdd2';
  ctx.font = '700 4.7px Arial,Helvetica,sans-serif';
  ctx.fillText(eFitText(a.sub, 142, '700 4.7px Arial,Helvetica,sans-serif'), 8, 248.5);
  ctx.strokeStyle = 'rgba(121,151,163,.20)';
  ctx.beginPath();
  ctx.moveTo(156, 224);
  ctx.lineTo(156, 253);
  ctx.moveTo(333, 224);
  ...
  ctx.font = '900 7.8px Arial,Helvetica,sans-serif';
  ctx.fillText(hz.main, 342, 237);
  ctx.fillStyle = '#aebfc5';
  ctx.font = '700 4.5px Arial,Helvetica,sans-serif';
  ctx.fillText(eFitText(hz.sub, 105, '700 4.5px Arial,Helvetica,sans-serif'), 342, 248.5);
}
function eVignette() {
  const g = ctx.createRadialGradient(CFG.width * .52, CFG.height * .48, CFG.height * .28, CFG.width * .52, CFG.height * .48, CFG.width * .72);
  g.addColorStop(.55, 'rgba(0,0,0,0)');
  g.addColorStop(1, 'rgba(0,0,0,.16)');
  ctx.fillStyle = g;
  ctx.fillRect(0, E_MAP_TOP, CFG.width, E_MAP_BOTTOM - E_MAP_TOP);
}
function ePerf(ms) {
  ...
  state.perf.last = ms;
  state.perf.samples.push(ms);
  if (state.perf.samples.length > 180) state.perf.samples.shift();
}
function eWarmCurrentView() {
  const t = performance.now();
  eBase();
  eLines();
  for (const f of state.frames) eRadarCanvas(f);
  eCells();
  if (state.severe.lightning) ePeak(state.severe.lightning, view(), 70, 3);
  if (state.severe.mesh) ePeak(state.severe.mesh, view(), 25.4, 2);
