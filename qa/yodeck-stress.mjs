...
const base=process.env.RDR_URL||'http://127.0.0.1:8765/',outDir=process.env.RDR_OUT||'/tmp/rdr-yodeck-stress',chrome=process.env.CHROME,legacyDecompression=process.env.RDR_LEGACY_DECOMPRESSION==='1';
...
const result={viewport:[456,257],cpuThrottle:4,legacyDecompression,views:{}};
const setup=async page=>{if(legacyDecompression)await page.evaluateOnNewDocument(()=>Object.defineProperty(window,'DecompressionStream',{value:undefined,configurable:true}));const cdp=await page.target().createCDPSession();await cdp.send('Emulation.setCPUThrottlingRate',{rate:4});await cdp.send('Network.enable');await cdp.send('Network.setCacheDisabled',{cacheDisabled:true})};
...
try{
  for(const id of ['home','metro','florida','regional']){
    ...
    if(d.viewport[0]!==456||d.viewport[1]!==257||d.panel[0]!==456||d.panel[1]!==257||d.radarLoading||d.backfillLocked||(legacyDecompression&&d.decompressionStream!=='undefined')||errors.length)throw new Error(`${id}: ${JSON.stringify(d)}`);
  }
  const page=await browser.newPage();await page.setViewport({width:456,height:257,deviceScaleFactor:1});await setup(page);const errors=[];page.on('pageerror',e=>errors.push(e.message));
  ...
  if(result.full.radarLoading||result.full.backfillLocked||result.full.frames<3||!result.full.severe.lightning||!result.full.severe.mesh||(legacyDecompression&&result.full.decompressionStream!=='undefined')||errors.length)throw new Error(`full: ${JSON.stringify(result.full)}`);
