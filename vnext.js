...
function vHaloText(text, x, y, {
  font = '700 6px Arial,Helvetica,sans-serif',
  fill = '#eef5f7',
  align = 'center',
  alpha = .98,
  halo = 'rgba(1,6,9,.78)',
...
function vCityLabels(occ) {
  const def = view(),
    base = def.id === 'home' ? 7.0 : def.id === 'metro' ? 6.7 : def.id === 'florida' ? 6.25 : 5.9,
    max = def.id === 'home' ? 6 : def.id === 'metro' ? 6 : def.id === 'florida' ? 7 : 6;
  let drawn = 0;
  for (const [name, lat, lon] of def.labels) {
    var _E_LABEL_OFF$def$id;
    ...
    const p = ePoint(lat, lon, def),
      off = ((_E_LABEL_OFF$def$id = E_LABEL_OFF[def.id]) === null || _E_LABEL_OFF$def$id === void 0 ? void 0 : _E_LABEL_OFF$def$id[name]) || [0, 0],
      x = p.x + off[0],
      y = p.y + off[1];
    if (!eInMap(y, 6)) continue;
    const font = `800 ${base}px Arial,Helvetica,sans-serif`;
    ctx.font = font;
    const w = Math.ceil(ctx.measureText(name).width) + 4,
      h = 9.5,
      b = ePlace(occ, w, h, [[x - w / 2, y - h / 2], [x - w / 2, y - h - 10], [x - w / 2, y + 3]]);
    if (!b) continue;
    vHaloText(name, b.x + b.w / 2, b.y + b.h / 2 + .1, {
      font,
      fill: 'rgba(238,246,249,.96)',
      halo: 'rgba(0,6,10,.78)',
      haloRadius: .72
    });
    drawn++;
...
function vLightningAtHome() {
  var _field$views;
  const field = state.severe.lightning,
    a = field === null || field === void 0 || (_field$views = field.views) === null || _field$views === void 0 ? void 0 : _field$views.home;
  if (!a) return null;
  const def = CFG.views[0],
    ...
    cy = Math.round(p.y);
  let best = null;
  for (let y = Math.max(0, cy - 4); y <= Math.min(CFG.height - 1, cy + 4); y++) for (let x = Math.max(0, cx - 4); x <= Math.min(CFG.width - 1, cx + 4); x++) {
    const q = a[y * CFG.width + x];
    if (q !== MISSING && (best === null || q > best)) best = q;
  }
...
function vNearestLightning(threshold = 30) {
  ...
  for (const id of ['home', 'metro']) {
    ...
    const def = CFG.views.find(v => v.id === id),
      a = (_field$views2 = field.views) === null || _field$views2 === void 0 ? void 0 : _field$views2[id];
    if (!a) continue;
    const step = id === 'home' ? 2 : 3;
    for (let y = E_MAP_TOP; y < E_MAP_BOTTOM; y += step) for (let x = 0; x < CFG.width; x += step) {
      const q = a[y * CFG.width + x];
      ...
      if (d2 < bestD2) {
        bestD2 = d2;
        best = _objectSpread(_objectSpread({}, p), {}, {
          value: q / 10
        });
      }
    }
  ...
  return _objectSpread(_objectSpread({}, best), {}, {
    miles: haversineMiles(CFG.home, best),
    dir: dir8(bearing(CFG.home, best))
  });
}
function vBolt(x, y, scale = 1, alpha = 1) {
  ...
  ctx.globalAlpha = alpha;
  ctx.shadowColor = 'rgba(255,241,111,.72)';
  ctx.shadowBlur = 4.5;
...
function vLightningSymbols(occ, now = performance.now()) {
  const def = view(),
    field = state.severe.lightning,
    maxCount = def.id === 'home' ? 3 : def.id === 'metro' ? 4 : def.id === 'florida' ? 5 : 4,
    peaks = ePeak(field, def, 35, maxCount);
  let drawn = 0;
  for (let i = 0; i < peaks.length; i++) {
    const q = peaks[i];
    if (!eInMap(q.y, 7)) continue;
    const pulse = .78 + .22 * Math.sin(now / 310 + i * 1.7),
      s = q.value >= 70 ? 1.1 : .88;
    ctx.save();
    ctx.strokeStyle = `rgba(255,241,111,${.22 + .18 * pulse})`;
    ctx.lineWidth = .55;
    ctx.beginPath();
    ctx.arc(q.x, q.y, 4.5 + q.value / 100 * 3, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
    vBolt(q.x, q.y, s, pulse);
    drawn++;
  }
  const ux = vEnsureUx();
  ux.lightningSymbols = drawn;
  ux.lightningMode = 'mrms-probability';
}
function vCentroidAround(frame, def, cx, cy, radiusPx = 24, threshold = 28) {
  var _frame$views;
  const a = frame === null || frame === void 0 || (_frame$views = frame.views) === null || _frame$views === void 0 ? void 0 : _frame$views[def.id];
  if (!a) return null;
  let sw = 0,
    sx = 0,
    sy = 0,
  ...
  if (count < 4 || !sw) return null;
  const x = sx / sw,
    y = sy / sw,
    p = pixelLatLon(x, y, def);
  return {
    x,
    y,
    lat: p.lat,
...
function vStormTracks(def = view()) {
  ...
  const old = state.frames.at(-2),
    cur = state.frames.at(-1),
    key = `${old.key}|${cur.key}|${def.id}|vnext1`;
  if (V_MOTION_CACHE.has(key)) return V_MOTION_CACHE.get(key);
  const hours = (cur.time - old.time) / 3600000;
  if (!(hours > 0)) {
    V_MOTION_CACHE.set(key, []);
    return [];
  }
  const max = def.id === 'home' ? 2 : def.id === 'metro' ? 3 : def.id === 'florida' ? 3 : 2,
    radius = def.id === 'home' ? 24 : def.id === 'metro' ? 28 : def.id === 'florida' ? 20 : 12,
    cores = eCells(cur, def).filter(c => c.value >= 45 && eInMap(c.y, 10)).slice(0, 8),
    out = [];
  for (const c of cores) {
    if (out.some(o => (o.x - c.x) ** 2 + (o.y - c.y) ** 2 < 42 ** 2)) continue;
    const a = vCentroidAround(old, def, c.x, c.y, radius),
      b = vCentroidAround(cur, def, c.x, c.y, radius);
    if (!a || !b) continue;
    const miles = haversineMiles(a, b),
      mph = miles / hours;
    if (mph < 5 || mph > 85) continue;
  ...
  if (!out.length && ['home', 'metro'].includes(def.id) && state.motion && Number.isFinite(state.motion.mph)) {
    const c = cores[0];
    if (c) out.push({
      x: c.x,
      y: c.y,
      mph: state.motion.mph,
      bearing: state.motion.bearing,
      dir: state.motion.dir,
      value: c.value
    });
  }
  V_MOTION_CACHE.set(key, out);
  while (V_MOTION_CACHE.size > 20) V_MOTION_CACHE.delete(V_MOTION_CACHE.keys().next().value);
  return out;
}
function vStormMotionArrows(occ) {
  const def = view(),
    tracks = vStormTracks(def);
  let drawn = 0;
  for (const t of tracks) {
    const r = t.bearing * Math.PI / 180,
      len = def.id === 'home' ? 17 : def.id === 'metro' ? 19 : 22,
      dx = Math.sin(r) * len,
      dy = -Math.cos(r) * len,
      x1 = t.x + dx * .18,
      y1 = t.y + dy * .18,
      x2 = t.x + dx,
      y2 = t.y + dy;
    if (!eInMap(y1, 7) || !eInMap(y2, 7)) continue;
    ctx.save();
    ctx.strokeStyle = 'rgba(132,226,249,.82)';
    ctx.fillStyle = 'rgba(132,226,249,.92)';
    ctx.lineWidth = .85;
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, y2);
    ...
    ctx.beginPath();
    ctx.moveTo(x2, y2);
    ctx.lineTo(x2 - Math.cos(ang - .55) * ah, y2 - Math.sin(ang - .55) * ah);
    ctx.lineTo(x2 - Math.cos(ang + .55) * ah, y2 - Math.sin(ang + .55) * ah);
    ctx.closePath();
    ctx.fill();
    ctx.restore();
    if (def.id !== 'regional' && drawn < 2) {
      const text = `${t.dir} ${t.mph}`,
        font = '800 4.7px Arial,Helvetica,sans-serif';
      ctx.font = font;
      const w = Math.ceil(ctx.measureText(text).width) + 3,
        h = 7,
        b = ePlace(occ, w, h, [[x2 + 3, y2 - 4], [x2 - w - 3, y2 - 4], [x2 + 3, y2 + 2]]);
      if (b) vHaloText(text, b.x + b.w / 2, b.y + b.h / 2, {
        font,
        fill: '#8fe7fb',
        halo: 'rgba(0,7,10,.78)',
        haloRadius: .65
  ...
  const ux = vEnsureUx();
  ux.motionArrows = drawn;
}
function vStormFocus(occ) {
  const def = view(),
    limit = def.id === 'home' ? 1 : def.id === 'metro' ? 2 : def.id === 'florida' ? 3 : 2;
  let shown = 0;
  for (const c of eCells()) {
    if (shown >= limit || c.value < 50 || !eInMap(c.y, 10)) continue;
    const accent = c.value >= 60 ? '#d852bc' : c.value >= 55 ? '#f04c66' : '#f56a43',
      label = `${Math.round(c.value)} dBZ`;
    ctx.save();
    ctx.strokeStyle = accent;
    ctx.lineWidth = .85;
    ctx.beginPath();
    ctx.arc(c.x, c.y, 3.7, 0, Math.PI * 2);
    ctx.stroke();
    ctx.restore();
    ctx.font = '800 5.1px Arial,Helvetica,sans-serif';
    const w = Math.ceil(ctx.measureText(label).width) + 7,
      h = 9.5,
      b = ePlace(occ, w, h, [[c.x + 6, c.y - 11], [c.x + 6, c.y + 3], [c.x - w - 6, c.y - 11]]);
    if (!b) continue;
    ctx.save();
    ctx.fillStyle = 'rgba(2,8,11,.58)';
    eRound(ctx, b.x, b.y, w, h, 2);
    ctx.fill();
    ctx.strokeStyle = accent;
    ctx.globalAlpha = .75;
    ctx.lineWidth = .55;
    ctx.stroke();
    ctx.restore();
    vHaloText(label, b.x + w / 2, b.y + h / 2, {
      font: '800 5.1px Arial,Helvetica,sans-serif',
      fill: '#f5f8f9',
      halo: 'rgba(0,5,8,.72)',
      haloRadius: .55
    });
...
function vExpiry(f) {
  const p = (f === null || f === void 0 ? void 0 : f.properties) || {},
    d = new Date(p.ends || p.expiration || '');
...
function vAlertPlace(item) {
  if (!(item !== null && item !== void 0 && item.c)) return '';
  const labels = view().labels || [];
  let best = null,
    dist = Infinity;
  for (const [name, lat, lon] of labels) {
    const d = haversineMiles(item.c, {
      lat,
      lon
    });
    if (d < dist) {
  ...
  const cap = view().id === 'regional' ? 240 : view().id === 'florida' ? 130 : view().id === 'metro' ? 60 : 30;
  return dist <= cap ? best : '';
}
function vAlertSummary() {
  const alerts = eWarningsInView();
  if (!alerts.length) return null;
  const local = localAlert(),
    urgent = alerts.find(x => x.meta.isWarning),
    localIn = local ? alerts.find(x => x.f === local) : null,
    primary = urgent || localIn || alerts[0],
    same = alerts.filter(x => x.meta.code === primary.meta.code).length,
    place = primary.f === local ? 'HOME' : vAlertPlace(primary),
    expires = vExpiry(primary.f);
  return {
    alerts,
    primary,
    meta: primary.meta,
...
function vWarningLabels(occ) {
  const def = view(),
    max = def.id === 'home' ? 2 : def.id === 'metro' ? 3 : def.id === 'florida' ? 4 : 3;
  let labels = 0;
  for (const item of eWarningsInView()) {
    if (labels >= max) break;
    const {
      c,
      meta
    } = item;
    if (!meta.isWarning || meta.isAdvisory || meta.isWatch) continue;
    const p = ePoint(c.lat, c.lon);
    if (!eInMap(p.y, 8)) continue;
    const font = '900 5.4px Arial,Helvetica,sans-serif';
    ctx.font = font;
    const w = Math.ceil(ctx.measureText(meta.code).width) + 4,
      h = 8,
      b = ePlace(occ, w, h, [[p.x + 3, p.y - 9], [p.x - w - 3, p.y - 9], [p.x + 3, p.y + 2]]);
    if (!b) continue;
    vHaloText(meta.code, b.x + b.w / 2, b.y + b.h / 2, {
      font,
      fill: meta.color,
      halo: 'rgba(0,5,8,.82)',
      haloRadius: .75
...
function vHomeText() {
  const h = state.home,
    ...
    e = h.eta,
    local = localAlert(),
    lm = local ? alertMeta(local) : null,
    ltg = vNearestLightning(30),
    extras = [];
  if (local) extras.push(`HOME ${lm.code}`);else if (ltg && ltg.miles <= 25) extras.push(`LTG ${Math.max(1, Math.round(ltg.miles))} MI ${ltg.dir}`);
  if (h.status !== 'DRY') {
    const dbz = Number.isFinite(h.dbz) ? `${Math.round(h.dbz)} dBZ` : '';
    return {
      main: h.status,
      sub: [dbz && `AT HOME ${dbz}`, ...extras].filter(Boolean).join('  •  ')
    };
  }
  if (n) return {
    main: 'DRY',
  ...
  return {
    main: 'DRY',
    sub: ['NO RAIN NEAR HOME', ...extras].join('  •  ')
  };
}
function vHazardText() {
  const sum = vAlertSummary();
  if (sum) {
    const main = `${sum.meta.code}${sum.same > 1 ? ` ${sum.same}` : ''}`,
      name = sum.meta.code === 'SVR' ? 'SEVERE T-STORM' : sum.meta.code === 'TOR' ? 'TORNADO WARNING' : sum.meta.code === 'FFW' ? 'FLASH FLOOD' : sum.meta.name.toUpperCase(),
      where = sum.place ? `${sum.place}` : 'IN VIEW',
      when = sum.expires ? `${sum.expires}` : '';
    return {
      main,
      sub: [name, where, when].filter(Boolean).join('  •  '),
      accent: sum.meta.color
    };
  }
  const ltg = fieldMax(state.severe.lightning),
    near = vNearestLightning(30),
    mesh = fieldMax(state.severe.mesh),
    tropical = view().id === 'regional' && state.tropics.length;
  if (ltg != null && ltg >= 30) return {
    main: `LTG ${Math.round(ltg)}%`,
    sub: near ? `RISK  •  ${Math.max(1, Math.round(near.miles))} MI ${near.dir}` : 'LIGHTNING RISK IN VIEW',
    accent: '#fff27a'
  };
  if (mesh != null && mesh >= 25.4) return {
    main: `HAIL ${(mesh / 25.4).toFixed(1)}\"`,
    sub: 'MRMS MESH IN VIEW',
    accent: '#ff84d7'
  };
  if (tropical) return {
    main: 'TROPICS',
    sub: 'NHC FEATURES ACTIVE',
    accent: '#ffd160'
  };
  return {
    main: 'NONE',
...
function vHeader() {
  ...
  if (sum) {
    const alert = `${sum.local ? 'HOME ' : ''}${sum.meta.code}${sum.same > 1 ? ` ${sum.same}` : ''}${sum.place && !sum.local ? `  •  ${sum.place}` : ''}${sum.expires ? `  •  ${sum.expires}` : ''}`;
...
function vFooter() {
  ...
  ctx.fillRect(0, E_FOOT_TOP, CFG.width, CFG.height - E_FOOT_TOP);
  ctx.strokeStyle = 'rgba(123,154,166,.27)';
  ctx.lineWidth = .55;
  ctx.beginPath();
...
function vAlertSummaryState() {
  ...
  state.alertUi = _objectSpread(_objectSpread({}, state.alertUi || {}), {}, {
    summary: !!sum,
    count: (sum === null || sum === void 0 ? void 0 : sum.count) || 0,
    primary: (sum === null || sum === void 0 || (_sum$meta = sum.meta) === null || _sum$meta === void 0 ? void 0 : _sum$meta.code) || null,
    local: !!(sum !== null && sum !== void 0 && sum.local),
    placement: 'header-footer'
  });
  const ux = vEnsureUx();
  ux.alertPanel = 'header-footer';
  ux.mapAlertBox = false;
}
function vRender(now = performance.now()) {
  const t0 = performance.now();
  if (typeof rdrSetMainTransform === 'function') rdrSetMainTransform();
  ctx.fillStyle = '#04131b';
  ctx.fillRect(0, 0, CFG.width, CFG.height);
  const ux = vEnsureUx();
  ux.lightningSymbols = 0;
  ux.motionArrows = 0;
  ux.cityLabels = 0;
  ux.warningLabels = 0;
  ux.advisoryLabels = 0;
  if (state.frames.length) {
    ...
    eDrawTropics();
    const occ = eOcc();
    vLightningSymbols(occ, now);
    eDrawWarnings();
    vStormMotionArrows(occ);
    eNearestRain();
    vStormFocus(occ);
    vWarningLabels(occ);
    vCityLabels(occ);
